...
// POST — send a test reminder email directly with provided data
// This works WITHOUT MongoDB — reads reminder data from request body
export async function POST(req: NextRequest) {
  ...
  for (const r of reminders) {
    ...
    for (const day of reminderDays) {
      ...
      if (!isToday && !force) {
        skipped.push(`${r.scholarshipTitle} — day ${day} not today (${reminderDate.toDateString()})`);
        continue;
      }
      ...
      if (!force && currentMins < targetMins) {
        skipped.push(`${r.scholarshipTitle} — time not reached yet (${targetMins} mins, now ${currentMins})`);
        continue;
      }
      ...
      await transporter.sendMail({
        ...
        to:   r.studentEmail,
        subject: daysLeft <= 1 ? `⏰ Last Day Tomorrow — ${r.scholarshipTitle}` : `📅 ${daysLeft} Days Left — ${r.scholarshipTitle}`,
        html: `
          <div style="font-family:sans-serif;max-width:480px;margin:auto;border-radius:16px;overflow:hidden;border:1px solid #e2e8f0">
            ...
            <div style="padding:24px;background:#fff">
              ...
              <p style="color:#374151;font-size:15px;margin:0 0 20px">📅 Only <b>${daysLeft} days left</b> to apply.</p>
              ...
              <p style="color:#9ca3af;font-size:12px;margin:0 0 20px">⏰ Scheduled for <b style="color:#374151">${timeStr}</b></p>
              ${r.applyLink ? `<a href="${r.applyLink}" style="display:block;background:linear-gradient(135deg,#16a34a,#15803d);color:#fff;text-align:center;padding:14px;border-radius:12px;font-weight:700;font-size:15px;text-decoration:none">Apply Now on Official Site →</a>` : ""}
            </div>
          </div>
        `,
      });
      sent.push(`✅ ${r.studentEmail} — ${r.scholarshipTitle}`);
    }
  }
